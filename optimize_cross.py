...
# To numpy data
time_vales = np.array(IS_data.index,dtype = int)
...
# Initirizaiton of parameters
xs_tmp  = tf.Variable(np.ones(Np-1),dtype=tf.float32)
xs =  6000 * xs_tmp
a = tf.Variable(0.5 * np.ones(Np-1),dtype=tf.float32)

ys_tmp  = tf.Variable(np.ones(Np-1),dtype=tf.float32)
ys =  10 * ys_tmp
b  = tf.Variable( 5*np.ones(Np-1),dtype=tf.float32)

c =  tf.Variable(1.0, dtype=tf.float32)

# Initizization of placeholders
_IS = tf.placeholder(tf.float32, shape = [9,Np-1])
_WBC = tf.placeholder(tf.float32, shape = [9,Np-1])
...
for i_learn in range(Np):
    #Initialization weights
    sess.run(tf.global_variables_initializer())
    # Setting learning data
    IS_learn  = np.hstack([IS_values[:,:i_learn],IS_values[:,i_learn+1:]])
    WBC_learn = np.hstack([WBC_values[:,:i_learn],WBC_values[:,i_learn+1:]])
    keys_learn = np.hstack([IS_data.keys()[:i_learn],IS_data.keys()[i_learn+1:]])
    # learning
    for  i in range(N):
    ...
    learned_data.append(pd.DataFrame(learned_values,index =['xs','ys','a','b','c','loss_data'], columns=keys_learn ))
    learned_data[i_learn].to_csv('intermediate_data/learned_data{0:02d}.csv'.format(i_learn))

    print('{0:02d} th calc is finished.'.format(i_learn))
